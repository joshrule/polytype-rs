use crate::{
    atype::{
        parser::{parse_type, parse_typeschema},
        Substitution, TypeContext,
    },
    Name, Source,
};
...
impl<'ctx, N: Name> Type<'ctx, N> {
    ...
    ///     let n_char = ctx.intern_name("char");
    ///     let n_bool = ctx.intern_name("bool");
    ///     let t_char = ctx.intern_tcon(n_char, &[]);
    ///     let t_int = ctx.intern_tcon(n_int, &[]);
    ///     let t_bool = ctx.intern_tcon(n_bool, &[]);
    ///     let t = ctx.arrow_slice(&[t_char, t_int, t_bool]);
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, Substitution, TypeContext, Type, Variable};
    /// # use polytype::Source;
    /// with_ctx(10, |ctx: TypeContext<'_>| {
    ...
    ///     let t_list_0 = ctx.intern_tcon(n_list, &[t_0]);
    ///
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, TypeContext, Type, Variable};
    /// with_ctx(10, |ctx: TypeContext<'_>| {
    ...
    pub fn parse(ctx: &TypeContext<'ctx, N>, s: &str) -> Result<Ty<'ctx, N>, ()> {
        parse_type(ctx, s)
    }
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, TypeContext, Type};
    /// with_ctx(10, |ctx: TypeContext<'_>| {
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, TypeContext, Type, UnificationError};
    /// with_ctx(10, |ctx: TypeContext<'_>| {
    ///     let t1 = Type::parse(&ctx, "int -> t0").expect("t1");
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, TypeContext, Type, UnificationError, Variable};
    /// with_ctx(10, |ctx: TypeContext<'_>| {
        ...
        if s != t {
            match (s, t) {
                ...
                (Type::Constructed(n1, a1), Type::Constructed(n2, a2)) => {
                    if n1 != n2 {
                        return Err(UnificationError::Failure(s, t));
                    } else {
...
impl<'ctx, N: Name> TypeSchema<'ctx, N> {
    ...
    ///
    /// ```
    /// # use polytype::atype::{with_ctx, TypeContext, TypeSchema, Variable};
    /// with_ctx(10, |ctx: TypeContext<'_>| {
    ...
    fn substitute(tp: Ty<'ctx, N>, sub: &Substitution<'ctx, N>) -> Ty<'ctx, N> {
        match *tp {
            ...
            Type::Variable(v) => sub.get(v).unwrap_or_else(|| tp),
        }
    }
    ...
    /// [`Display`]: https://doc.rust-lang.org/std/fmt/trait.Display.html
    /// [`TypeSchema`]: enum.TypeSchema.html
    pub fn parse(ctx: &TypeContext<'ctx, N>, s: &str) -> Result<Schema<'ctx, N>, ()> {
